...
public class EmotionAnalyzer {
	...
	/**
	 * 빈출단어 반환하기
	 * @param KOMORAN 형태소분석으로 정리된 명사List
	 * @return String 빈출단어
	 */
	     ...
	     * karlo API에 전달될 파라미터
	     * @param KOMORAN 형태소분석으로 정리된 명사List
	     * @return 중복된 값이 많은 명사가 최대 5개 들어있는 명사리스트 
	     */
//	public static List<String> wordsForAi (List<String> nouns) {
//		List<String> uniqueWords = new ArrayList<>(); // 중복이 없는 단어들을 저장할 리스트
//		Map<String, Integer> wordCountMap = new HashMap<>(); // 단어별 중복 개수를 저장할 맵
//
//		for (String word : nouns) {
//			if (!uniqueWords.contains(word)) { // 중복이 없는 경우
//				uniqueWords.add(word); // 중복이 없는 단어를 리스트에 추가
//			} else if (!wordCountMap.containsKey(word)) { // 중복된 경우
//				wordCountMap.put(word, 2); // 중복된 단어와 개수를 맵에 저장 (처음 중복되었으므로 개수는 2로 초기화)
//			} else { // 이미 중복된 단어가 있는 경우
//				int count = wordCountMap.get(word); // 이미 저장된 중복된 단어의 개수를 가져옴
//				if (count < 5) { // 중복된 단어의 개수가 5개 미만인 경우에만
//					wordCountMap.put(word, count + 1); // 중복된 단어의 개수를 1 증가시켜 맵에 저장
//				}
//			}
//		}
//
//		// 중복이 많이 된 단어 순서로 5개까지만 uniqueWords 리스트에 추가
